...
import blanco.maldonado.mendoza.apiproductos.mappers.toDto
...
@RestController
//@RequestMapping(APIConfig.API_PATH + "/api/productos")  ento para cuando cojamos la path de el fichero config
...
@Autowired constructor(
     ...
     */
    @GetMapping("")
    suspend fun findAllProductos(): ResponseEntity<Flow<ProductosDTO>> = withContext(Dispatchers.IO) {
     ...
     */
    @GetMapping("/{id}")
    suspend fun findProductById(@PathVariable id: UUID): ResponseEntity<ProductosDTO> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            return@withContext ResponseEntity.ok(res)
        }catch (e : ProductosNotFoundException){
    ...
    suspend fun findProductByCategoria(@PathVariable categoria: String): ResponseEntity<Flow<ProductosDTO>> = withContext(Dispatchers.IO) {
     ...
     */
    @GetMapping("/{nombre}")
    suspend fun findProductById(@PathVariable nombre: String): ResponseEntity<ProductosDTO> = withContext(Dispatchers.IO) {
        logger.info { "Obteniendo producto por id" }
     ...
     */
    @PostMapping("")
    suspend fun createProduct(@Valid @RequestBody productoDto : ProductoRequestDto): ResponseEntity<ProductosDTO> = withContext(Dispatchers.IO) {
        ...
        try {
            val p = productoDto.validate().toProducto()
     ...
     */
    @PutMapping("/{id}")
    suspend fun updateProduct(@PathVariable id : UUID, @Valid, @RequestBody productoDto : ProductosDTO
    ): ResponseEntity<ProductosDTO> = withContext(Dispatchers.IO) {
        ...
        }catch (e:ProductoNotFoundExcepcion){
            throw ResponseStatusException(HttpStatus.NOT_FOUND, e.message)
        }catch (e:ProductoBadRequestExcepcion){
            throw ResponseStatusException(HttpStatus.BAD_REQUEST, e.message)
        }
    ...
    @DeleteMapping("/{id}")
    suspend fun deleteProduct(@PathVariable id : UUID): ResponseEntity<ProductosDTO> = withContext(Dispatchers.IO) {
